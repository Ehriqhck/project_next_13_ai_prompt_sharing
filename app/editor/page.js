...
import { ShowEditorPanelContext, ShowViewerPanelContext, SelectedEditorDeviceContext, SelectedEditorDeviceViewOrientationContext, SelectContext, SelectedInputTableInputContext } from '@components/Provider';
...
const page = () => {
  ...
  const deviceAxisRenderer = (item, itemIndex) => (
    <Button type="device_axis" className='mx-[4px]' onClick={() => setActiveIndexactiveIndexDeviceAxis(itemIndex)}>
      {getDeviceAxis(item)}
      <span className="small-text flex self-center">{item.name}</span>
    </Button>
  );
  ...
  useEffect(() => {
    try {
      setIsLoading(true);
      setCurrentSelectedEditorDevice(getSelectedEditorDevice(sessionStorage.getItem('selectedEditorDevice')));
  ...
  }, [selectedEditorDevice, selectedEditorDeviceViewOrientation, SelectedEditorDeviceViewOrientationContext])
  ...
  const getSelectedEditorDevice = (selectedDevice) => {
    ...
    switch (sessionStorage.getItem('selectedEditorDevice')) {
      case "VKB_GLADIATOR_NXT_EVO_RIGHT":
        return (
          <Device_VKB_GLADIATOR_NXT_EVO_RIGHT className="min-w-[330px]" view={sessionStorage.getItem('selectedEditorDevice')} />
      ...
      case "VKB_GLADIATOR_NXT_EVO_LEFT":
        return (
          <Device_VKB_GLADIATOR_NXT_EVO_LEFT className="min-w-[330px]" view={sessionStorage.getItem("deviceViewOrientation")} />
        )
        break;
      default:
        return (
